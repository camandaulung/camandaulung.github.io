 ...
 * thay vì tự đổi bản giữa lúc đang chơi dở một màn.
 *
 * Tên cache PHẢI mang tên game: cả cổng minigame nằm chung một tên miền, hai game
 * trùng tên cache sẽ xoá cache của nhau ở bước `activate` (chỗ đó xoá mọi key khác
 * CACHE hiện tại). Bẫy: chuỗi này là nơi DUY NHẤT quyết định tên cache thật —
 * `build.mjs` chỉ nhắc tên game ở chú thích và một dòng console.log, sửa mỗi bên đó
 * là được kết quả xanh giả.
 */

const CACHE = 'neon-dash-608d0475';
...
const SHELL = [
  ...
  "./third-party/three-r180/three.core.min.js",
  "./third-party/three-r180/three.module.min.js",
...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(CACHE)
      ...
      // Đã xảy ra thật trên bản phát hành của cờ vua (sự cố số 2 trong CLAUDE.md).
      .then(c => Promise.all(
